...
<div class="block" id="sidebarUser">
	...
	{if $isUserLoggedIn}
		{translate key="plugins.block.extendedUser.loggedInAs"}<br />
		<strong>{$loggedInUsername|escape}</strong>
		<ul>
			{if $hasOtherJournals}
				<li><a href="{url journal="index" page="user"}">{translate key="plugins.block.extendedUser.myJournals"}</a></li>
			{/if}
		...
		{else}
			<form method="post" action="{$userBlockLoginUrl}">
				<table>
					...
					<tr>
						<td colspan="2"><input type="checkbox" id="remember" name="remember" value="1" /> <label for="remember">{translate key="plugins.block.extendedUser.rememberMe"}</label></td>
					</tr>
					<tr>
