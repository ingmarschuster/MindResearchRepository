...
/**
 * @file plugins/importexport/paperPackageEdit/PaperPackageEditForm.inc.php
 *
 * Copyright (c) 2013 University of Potsdam, 2003-2012 John Willinsky
 * Distributed under the GNU GPL v2. For full terms see the file docs/COPYING.
 *
 * @class PaperPackageEditForm
 * @ingroup plugins_importexport_paperPackageEdit
 *
 * @brief Form for PaperPackageEdit one-page submission plugin
 */


import('lib.pkp.classes.form.Form');
class PaperPackageEditForm extends Form {
	 ...
	 * @param $plugin object
	 */
	function PaperPackageEditForm(&$plugin) {
                parent::Form($plugin->getTemplatePath() . 'index.tpl');

		$journal =& Request::getJournal();

                import('plugins.importexport.paperPackageEdit.FormValidatorFileType');
	        import('plugins.importexport.paperPackageEdit.FormValidatorHandle');
         	import('plugins.importexport.paperPackageEdit.FormValidatorHandleOrFile');
		$this->addCheck(new FormValidatorPost($this));
		$this->addCheck(new FormValidator($this, 'sectionId', 'required', 'author.submit.form.sectionRequired'));
                $this->addCheck(new FormValidatorHandle($this, 'submissionHandle', 'required', 'plugins.importexport.paperPackageEdit.submissionHandleIsWrong'));
                $this->addCheck(new FormValidatorHandle($this, 'supplHandle', 'required', 'plugins.importexport.paperPackageEdit.supplHandleIsWrong'));
		$this->addCheck(new FormValidatorHandleOrFile($this, 'submissionHandle', 'required', 'plugins.importexport.paperPackageEdit.submissionHandleOrFile', 'tempFileId'));
		$this->addCheck(new FormValidatorHandleOrFile($this, 'supplHandle', 'required', 'plugins.importexport.paperPackageEdit.supplHandleOrFile', 'tempSupplFileId'));
		$this->addCheck(new FormValidatorFileType($this, 'tempSupplFileId', 'required', 'plugins.importexport.paperPackageEdit.supplUnpackable'));
                $this->addCheck(new FormValidatorCustom($this, 'datePublished', 'required', 'plugins.importexport.paperPackageEdit.dateRequired', create_function('$destination, $form', 'return is_int($form->getData(\'datePublished\'));'), array(&$this)));
...
//		$this->addCheck(new FormValidatorCustom($this, 'destination', 'required', 'plugins.importexport.paperPackageEdit.issueRequired', create_function('$destination, $form', 'return $destination == \'queue\'? true : ($form->getData(\'issueId\') > 0);'), array(&$this)));
		$this->addCheck(new FormValidatorArray($this, 'authors', 'required', 'plugins.importexport.paperPackageEdit.authorRequiredFields', array('firstName', 'lastName')));
		...
		$this->addCheck(new FormValidatorLocale($this, 'title', 'required', 'author.submit.form.titleRequired'));
                $this->addCheck(new FormValidator($this, 'originalJournal', 'required', 'plugins.importexport.paperPackageEdit.originalJournalRequired'));
	}
...
*/
	...
	import('plugins.importexport.paperPackageEdit.PDF');
	$pdf = new PDF();
        $pdf->AddPage();
...
//		$abstr[$abs_key] .=  '  <p id="originalPub"> ' . $orig_journal[$abs_key]. ' </p> ';
	...
	function scheduleForPublication($articleId, $issueId) {
		...
		if ($issue) {
			...
			if ($publishedArticle) {
				error_log("OJS - PPEF: Es wird ein alter publishedArticle gelöscht.");
